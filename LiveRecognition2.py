...
while 1:
    ...
    for face_id, tracker in trackers.items():  # iterate over current trackers
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT  *  FROM studenttb where  RegisterNo='" + str(ss) + "'")
        ...
        if data:
            ...
            conn = mysql.connector.connect(user='root', password='', host='localhost', database='5collegestuatIOdb')
            cursor = conn.cursor()
            cursor.execute("select * from attentb where Date='" + str(date) + "' and Regno='" + str(ss) + "' and Attendance='OUT'")
            data = cursor.fetchone()
            if data is None:
                ...
                print(Mobile)
                sendmsg(Mobile, str(regno) + "," + str(name) + " Out Today")
            else:
